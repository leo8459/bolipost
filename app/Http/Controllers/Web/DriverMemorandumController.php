...
class DriverMemorandumController extends Controller
{
    public function show(Request $request, Driver $driver)
    {
        ...
        if ($request->user()?->role === 'conductor') {
            ...
            abort_if($driverId <= 0 || $driverId !== (int) $driver->id, 403);
        }

        $stored = trim((string) ($driver->memorandum_path ?? ''));
        abort_if($stored === '', 404, 'Memorandum no encontrado.');
        ...
        abort(404, 'Archivo de memorandum no encontrado en almacenamiento.');
    }
