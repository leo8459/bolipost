...
class PaquetesEms extends Component
{
    ...
    public $filtroOrigenTransito = '';
    public $filtroDestinoTransito = '';
    public $codEspecialDetalle = '';
    ...
    public function toggleCn38Generate(): void
    {
        $this->authorizePermission(self::ALMACEN_EMS_REPRINT_CN33_PERMISSION);
    ...
    public function cargarCn38DesdeCn33(): void
    {
        $this->authorizePermission(self::ALMACEN_EMS_REPRINT_CN33_PERMISSION);
        ...
        if ($despacho === '') {
            session()->flash('error', 'Ingresa el despacho (cod_especial) del CN-33 para cargar el CN-38.');
            return;
        }
    ...
    {
        $this->filtroOrigenTransito = strtoupper(trim((string) $value));
        $this->resetPage();
    }

    public function updatedFiltroDestinoTransito($value): void
    {
        $this->filtroDestinoTransito = strtoupper(trim((string) $value));
        $this->resetPage();
    }

    public function limpiarFiltrosTransito(): void
    {
        $this->filtroOrigenTransito = '';
        $this->filtroDestinoTransito = '';
        $this->search = '';
        $this->searchQuery = '';
        $this->resetPage();
    }

    public function openCodEspecialDetalle(string $codEspecial): void
    {
        $codEspecial = strtoupper(trim($codEspecial));

        if ($codEspecial === '' || $codEspecial === '-') {
            return;
        }

        $this->codEspecialDetalle = $codEspecial;
        $this->codEspecialDetalleRows = $this->codEspecialDetalleRows($codEspecial)
            ->map(fn ($row) => (array) $row)
            ->all();

        $this->dispatch('openCodEspecialDetalleModal');
    }
    ...
    public function openCn38PrintOptionsModal(): void
    {
        ...
        $this->showCn38PrintOptionsModal = true;
        $this->dispatch('openCn38PrintOptionsModal');
    }

    public function cerrarCn38OpcionesImpresion(): void
    {
        $this->showCn38PrintOptionsModal = false;
        $this->dispatch('closeCn38PrintOptionsModal');
    }
    ...
    protected function generarCn38EnFormato(string $formato)
    {
        ...
        $totalCantidad = (int) $rows->count();
        $generatedAt = collect($rows->pluck('updated_at'))
            ->filter()
            ->sortDesc()
            ->first() ?: now();
    ...
    public function mandarSeleccionadosContratosRegional(bool $confirmadoDestino = false)
    {
        ...
        if (!$this->ensureRegionalZeroWeightResolved([], $ids, [])) {
            session()->flash('error', 'Los paquetes con este codigo estan en peso 0. Por favor asigne el peso antes de mandar a regional.');
            return;
        }
    ...
    public function render()
    {
        ...
        if ($this->isAlmacenEms || $this->isEnTransitoEms || $this->isTransitoEms || $this->isVentanillaEms || $this->isDevolucionEms) {
            ...
            if ($this->canUseSelectedPreview) {
                ...
                $cn38DispatchSummaryRows = $this->buildCn38DispatchSummaryRows();
            }
    ...
    protected function buildCn38DispatchSummaryRows(): Collection
    {
        $idsEms = collect($this->selectedPaquetes)
            ->filter()
            ->map(fn ($id) => (int) $id)
        ...
        return $rows
            ->groupBy(fn ($row) => strtoupper(trim((string) ($row->despacho ?? 'SIN CN-33'))))
            ->map(function ($group, $despacho) {
                return (object) [
                    'despacho' => $despacho,
                    'registros' => $group->count(),
                    'peso_total' => (float) $group->sum(fn ($item) => (float) ($item->peso ?? 0)),
                ];
            })
            ->sortBy('despacho')
            ->values();
    ...
    protected function resolveCn38SelectionIds(string $despacho): array
    {
        $codigoNormalizado = strtoupper(trim($despacho));

        $idsEms = PaqueteEms::query()
    ...
    protected function buildCn38Rows(string $despacho): Collection
    {
        ...
        $emsRows = PaqueteEms::query()
            ->with(['user:id,empresa_id', 'user.empresa:id,nombre'])
            ->whereRaw('trim(upper(COALESCE(paquetes_ems.cod_especial, \'\'))) = trim(upper(?))', [$codigoNormalizado])
            ->orderBy('paquetes_ems.id')
            ->get($this->paqueteEmsColumns([
                'id',
                'codigo',
            ...
            ->map(function ($row) {
                return (object) [
                    ...
                    'remitente' => $this->composeCn33Remitente(
        ...
        $solicitudRows = SolicitudCliente::query()
            ...
            ->map(function ($row) {
                return (object) [
                    ...
                    'cantidad' => (int) ($row->cantidad ?? 1),
                    'peso' => (float) ($row->peso ?? 0),
                    'remitente' => (string) ($row->nombre_remitente ?? 'SIN REMITENTE'),
                    'observacion' => (string) ($row->observacion ?? ''),
                    'updated_at' => $row->updated_at ?? null,
                ];
            });
        ...
        return $rows
            ...
            ->map(function ($group, $despacho) {
                ...
                return (object) [
                    ...
                    'peso_total' => (float) $group->sum(fn ($item) => (float) ($item->peso ?? 0)),
                    'registros' => (int) $group->count(),
                    'updated_at' => $group->pluck('updated_at')->filter()->sortDesc()->first(),
                ];
            })
            ->sortBy('despacho')
            ->values();
    ...
    protected function almacenUnificadoQuery()
    {
        ...
        $contratosQuery = DB::table('paquetes_contrato')
            ...
            ->when(!$isModoRecibirRegional && !$isModoVentanilla && !$isModoEnTransito, function ($query) use ($userCity, $isModoEnTransito, $isModoDevolucion, $estadoAlmacenId, $estadoRecibidoId) {
                if ($userCity !== '') {
                    if ($isModoDevolucion) {
                        $query->where(function ($sub) use ($userCity, $estadoAlmacenId, $estadoRecibidoId) {
                            $sub->where(function ($q2) use ($userCity, $estadoAlmacenId) {
        ...
        $query = DB::query()
            ...
            ->when($this->isEnTransitoEms && $this->filtroOrigenTransito !== '', function ($query) {
                $query->whereRaw('trim(upper(origen)) = trim(upper(?))', [$this->filtroOrigenTransito]);
            })
        ...
        if ($this->isEnTransitoEms) {
            return DB::query()
                ->fromSub(
                    $query
                        ->selectRaw('almacen_mix.*')
                        ->selectRaw("
                            row_number() over (
                                partition by
                                    case
                                        when trim(coalesce(cod_especial, '')) = '' then codigo
                                        else trim(upper(cod_especial))
                                    end
                                order by created_at desc, record_id desc
                            ) as cod_especial_row_number
                        "),
                    'transito_grouped'
                )
                ->where('cod_especial_row_number', 1)
    ...
    protected function transitoFilterOptions(string $field): array
    {
        if (!$this->isEnTransitoEms || !in_array($field, ['origen', 'destino'], true)) {
            return [];
        }

        if ($field === 'destino') {
            return array_keys($this->transitoDepartamentoDestinoMap());
        }
        ...
        $emsQuery = DB::table('paquetes_ems')
            ...
            ->leftJoin('tarifario', 'tarifario.id', '=', 'paquetes_ems.tarifario_id')
            ->leftJoin('destino', 'destino.id', '=', 'tarifario.destino_id')
            ->where('paquetes_ems.estado_id', (int) $estadoTransitoId)
            ->selectRaw($field === 'origen'
                ? "coalesce(paquetes_ems.origen, '-') as value"
                : "coalesce(destino.nombre_destino, formulario.ciudad, paquetes_ems.ciudad, '-') as value"
            );

        $contratosQuery = DB::table('paquetes_contrato')
            ->where('paquetes_contrato.estados_id', (int) $estadoTransitoId)
            ->selectRaw($field === 'origen'
                ? "coalesce(paquetes_contrato.origen, '-') as value"
                : "coalesce(paquetes_contrato.destino, '-') as value"
            );

        $solicitudesQuery = DB::table('solicitud_clientes')
            ->leftJoin('destino', 'destino.id', '=', 'solicitud_clientes.destino_id')
            ->where('solicitud_clientes.estado_id', (int) $estadoTransitoId)
            ->selectRaw($field === 'origen'
                ? "coalesce(solicitud_clientes.origen, '-') as value"
        ...
        return DB::query()
            ...
            ->selectRaw('trim(upper(value)) as value')
    ...
    protected function transitoDepartamentoDestinoMap(): array
    {
        return [
            'BENI' => ['BENI', 'TRINIDAD'],
    ...
    protected function transitoDestinoAliases(string $destino): array
    {
        $destino = strtoupper(trim($destino));

        return $this->transitoDepartamentoDestinoMap()[$destino] ?? [$destino];
    }

    protected function codEspecialDetalleRows(string $codEspecial): Collection
    {
        ...
        $emsQuery = DB::table('paquetes_ems')
            ...
            ->whereRaw('trim(upper(coalesce(paquetes_ems.cod_especial, \'\'))) = ?', [$codigo])
            ->selectRaw("'EMS' as tipo")
            ->selectRaw("coalesce(nullif(trim(formulario.codigo), ''), paquetes_ems.codigo) as codigo")
            ->selectRaw("coalesce(paquetes_ems.origen, '-') as origen")
            ->selectRaw("coalesce(destino.nombre_destino, formulario.ciudad, paquetes_ems.ciudad, '-') as destino")
        ...
        $contratosQuery = DB::table('paquetes_contrato')
            ...
            ->whereRaw('trim(upper(coalesce(paquetes_contrato.cod_especial, \'\'))) = ?', [$codigo])
            ->selectRaw("'CONTRATO' as tipo")
            ->selectRaw('paquetes_contrato.codigo as codigo')
            ->selectRaw("coalesce(paquetes_contrato.origen, '-') as origen")
            ->selectRaw("coalesce(paquetes_contrato.destino, '-') as destino")
        ...
        $solicitudesQuery = DB::table('solicitud_clientes')
            ->leftJoin('destino', 'destino.id', '=', 'solicitud_clientes.destino_id')
            ->leftJoin('estados', 'estados.id', '=', 'solicitud_clientes.estado_id')
            ->whereRaw('trim(upper(coalesce(solicitud_clientes.cod_especial, \'\'))) = ?', [$codigo])
            ->selectRaw("'SOLICITUD' as tipo")
            ->selectRaw("coalesce(nullif(trim(solicitud_clientes.codigo_solicitud), ''), nullif(trim(solicitud_clientes.barcode), ''), 'SIN CODIGO') as codigo")
            ->selectRaw("coalesce(solicitud_clientes.origen, '-') as origen")
            ->selectRaw("coalesce(solicitud_clientes.ciudad, destino.nombre_destino, '-') as destino")
    ...
    protected function prepareRegionalPesoZeroData(array $idsEms, array $idsContratos, array $idsSolicitudes): void
    {
        ...
        if (!empty($idsEms)) {
            $items = $items->merge(
                PaqueteEms::query()
                    ...
                    ->orderBy('id')
                    ->get(['id', 'codigo', 'peso'])
                    ->map(function (PaqueteEms $paquete) use ($existingInputs) {
                        $key = 'ems_' . (int) $paquete->id;
        ...
        if (!empty($idsContratos)) {
            $items = $items->merge(
                RecojoContrato::query()
                    ...
                    ->orderBy('id')
                    ->get(['id', 'codigo', 'peso'])
                    ->map(function (RecojoContrato $contrato) use ($existingInputs) {
                        $key = 'contrato_' . (int) $contrato->id;

                        return [
                            'key' => $key,
                            'type' => 'contrato',
                            'id' => (int) $contrato->id,
                            'codigo' => (string) ($contrato->codigo ?: 'SIN CODIGO'),
                            'peso' => (float) ($contrato->peso ?? 0),
                            'input' => $existingInputs->get($key, ''),
                        ];
                    })
        ...
        if (!empty($idsSolicitudes)) {
            $items = $items->merge(
                SolicitudCliente::query()
                    ...
                    ->orderBy('id')
                    ->get(['id', 'codigo_solicitud', 'barcode', 'peso'])
                    ->map(function (SolicitudCliente $solicitud) use ($existingInputs) {
                        $key = 'solicitud_' . (int) $solicitud->id;
                        $codigo = $solicitud->codigo_solicitud ?: ($solicitud->barcode ?: 'SIN CODIGO');
        ...
        $this->regionalPesoZeroItems = $items->values()->all();
        $this->regionalPesoInputs = $items
            ->mapWithKeys(fn (array $item) => [$item['key'] => $item['input']])
            ->all();
    }
    ...
    protected function ensureRegionalZeroWeightResolved(array $idsEms, array $idsContratos, array $idsSolicitudes): bool
    {
        ...
        foreach ($this->regionalPesoZeroItems as $item) {
            $key = (string) ($item['key'] ?? '');
            $codigo = (string) ($item['codigo'] ?? 'SIN CODIGO');
            ...
            $rules['regionalPesoInputs.' . $key] = 'required|numeric|min:0.001';
            $messages['regionalPesoInputs.' . $key . '.required'] = 'Debes asignar el peso para ' . $codigo . '.';
            $messages['regionalPesoInputs.' . $key . '.numeric'] = 'El peso para ' . $codigo . ' debe ser numerico.';
            $messages['regionalPesoInputs.' . $key . '.min'] = 'El peso para ' . $codigo . ' debe ser mayor a 0.';
        }

        $validated = $this->validate($rules, $messages);
        $pesos = collect($validated['regionalPesoInputs'] ?? [])
            ->mapWithKeys(fn ($peso, $key) => [$key => round((float) $peso, 3)]);

        try {
            DB::transaction(function () use ($pesos) {
                foreach ($this->regionalPesoZeroItems as $item) {
                    $key = (string) ($item['key'] ?? '');
                    ...
                    if ($type === 'contrato') {
                        ...
                        $teniaTarifaAsignada = (int) ($contrato->tarifa_contrato_id ?? 0) > 0;
                        $contrato->peso = $peso;
                        $this->applyTarifaContratoPricing($contrato, $teniaTarifaAsignada);
                        continue;
                    }
            ...
            if (str_contains($message, '|')) {
                [$errorKey, $errorMessage] = explode('|', $message, 2);
                $this->addError('regionalPesoInputs.' . $errorKey, $errorMessage);
                return false;
            }
        ...
        $this->prepareRegionalPesoZeroData(
            $this->selectedPaquetesAsInt(),
            $this->selectedContratosAsInt(),
            $this->selectedSolicitudesAsInt()
        );

        return empty($this->regionalPesoZeroItems);
    }

    protected function selectedPaquetesAsInt(): array
    {
        return collect($this->selectedPaquetes)
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values()
    ...
    protected function selectedContratosAsInt(): array
    {
        return collect($this->selectedContratos)
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values()
    ...
    protected function selectedSolicitudesAsInt(): array
    {
        return collect($this->selectedSolicitudes)
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values()
