<div>
    ...
    <div class="plantilla-wrap">
        <div class="card card-app">
            <div class="header-app d-flex flex-column flex-md-row justify-content-between gap-3 align-items-md-center">
                <div>
                    <h4 class="fw-bold mb-0">Pesos</h4>
                </div>
                ...
                <div class="d-flex gap-2 align-items-center">
                    <input
                        ...
                        wire:model="search"
                    >
                    <button class="btn btn-outline-light2" type="button" wire:click="searchPesos">Buscar</button>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="muted small">
                        Total en pagina: <strong>{{ $pesos->count() }}</strong>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($pesos as $peso)
                                <tr>
                                    <td>
                                        <span class="pill-id">
                                            #{{ ($pesos->firstItem() ?? 0) + $loop->index }}
                                        </span>
                                    </td>
                                    <td>{{ $peso->peso_inicial }}</td>
                                    ...
                                    <td>
                                        ...
                                        <button wire:click="delete({{ $peso->id }})"
                                            ...
                                            onclick="return confirm('Seguro que deseas eliminar este peso?')">
                                            Borrar
                                        </button>
                                    </td>
                                </tr>
    ...
    <div class="modal fade" id="pesoModal" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <form wire:submit.prevent="save">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{ $editingId ? 'Editar peso' : 'Nuevo peso' }}
                        </h5>
...
<script>
    window.addEventListener('openPesoModal', () => {
        $('#pesoModal').modal('show');
    });

    window.addEventListener('closePesoModal', () => {
