<div>
    <style>
        #vehicle-location-map { height: 360px; border-radius: 8px; }
        #vehicleLocationPickerModal .modal-dialog { max-width: 900px; }
        #vehicle-view-map { height: 420px; border-radius: 8px; }
        #vehicleViewMapModal .modal-dialog { max-width: 980px; }
        #vehicle-form-side-map { height: 420px; border-radius: 10px; }
        #vehicle-side-map { height: 430px; border-radius: 10px; }
        .bitacora-shell {
            border-radius: 18px;
            overflow: hidden;
    ...
    (function () {
        if (!window.__vehicleKmSyncInitialized) {
            ...
            const syncKmSalida = () => {
                ...
                kmSalidaInput.dispatchEvent(new Event('input', { bubbles: true }));
                kmSalidaInput.dispatchEvent(new Event('change', { bubbles: true }));
            };
        ...
        function initMap() {
            ...
            map.on('click', async function (e) {
                ...
                selected = { lat, lng, name: placeName || 'Ubicacion seleccionada (Bolivia)' };
        ...
        document.addEventListener('click', function (event) {
            ...
            const { panelEl, mapEl, metaEl, helpEl } = getFormMapElements();
        ...
        function drawTripOnSideMap(payload) {
            ...
            if (bounds.length === 2) {
                L.polyline(bounds, { color: '#2563eb', weight: 4, opacity: 0.9 }).addTo(sideLayer);
            }
            if (bounds.length > 0) {
                sideMap.fitBounds(bounds, { padding: [24, 24] });
            } else {
