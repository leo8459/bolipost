<div>
    <style>
        ...
        #qr-reader-modal video { filter: contrast(1.15) brightness(1.08); }
        .fuel-table { min-width: 1300px; }
        .fuel-table th, .fuel-table td { white-space: nowrap; }
        .fuel-table--combined { min-width: 100%; }
        ...
        .fuel-tables-shell { margin-left: 0; margin-right: 0; }
        #location-map { height: 360px; border-radius: 8px; }
        #fuel-form-side-map { height: 420px; border-radius: 10px; }
        #locationPickerModal .modal-dialog { max-width: 900px; }
        #bitacora-view-map { height: 420px; border-radius: 8px; }
        #bitacoraMapModal .modal-dialog { max-width: 980px; }
        .fuel-form-map-card {
    ...
    @else
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0 fuel-table fuel-table--combined bitacora-style">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr><td colspan="10" class="text-center py-4 text-muted">Sin registros de combustible</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0 fuel-table bitacora-style">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr><td colspan="9" class="text-center py-4 text-muted">Sin registros de bitacora</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0 fuel-table bitacora-style">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr><td colspan="6" class="text-center py-4 text-muted">Sin datos para combinar</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        @if($tableView === 'fuel')
            <div class="mt-4">{{ $fuelLogs->links() }}</div>
        @elseif($tableView === 'bitacora')
            <div class="mt-4">{{ $vehicleLogs->links() }}</div>
        @elseif($tableView === 'combined')
            <div class="mt-4">{{ $combinedRows->links() }}</div>
        @endif
    @endif
    ...
    (function () {
        ...
        function buildTargetFromInputId(inputId) {
            ...
            if (inputId === 'recorrido_inicio') {
                return { inputId, latId: 'latitud_inicio', lngId: 'logitud_inicio' };
            }
            if (inputId === 'recorrido_destino') {
                return { inputId, latId: 'latitud_destino', lngId: 'logitud_destino' };
            }
            if (inputId === 'recorrido_inicio_bitacora') {
                return { inputId, latId: 'latitud_inicio_bitacora', lngId: 'logitud_inicio_bitacora' };
            }
            if (inputId === 'recorrido_destino_bitacora') {
                return { inputId, latId: 'latitud_destino_bitacora', lngId: 'logitud_destino_bitacora' };
            }
            return null;
        }
        ...
        function updateFormMapStatus() {
            const { metaEl, helpEl } = getFormMapElements();
        ...
        function ensureFormSideMap() {
            const { mapEl, helpEl } = getFormMapElements();
            ...
            formSideMap.on('click', async function (e) {
                ...
                if (startMarker && endMarker) {
                    ...
                    if (!formTargetLine) {
                        formTargetLine = L.polyline(points, { color: '#2563eb', weight: 3, opacity: 0.9 }).addTo(formSideMap);
                    } else {
        ...
        function bindHandlersOnce() {
            ...
            document.addEventListener('click', function (event) {
                ...
                const { panelEl, mapEl } = getFormMapElements();
            ...
            document.addEventListener('click', function (event) {
                ...
                const { panelEl, metaEl, helpEl } = getFormMapElements();
