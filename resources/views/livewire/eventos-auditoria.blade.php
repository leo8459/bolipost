<div>
    ...
    <div class="plantilla-wrap">
        <div class="card card-app">
            <div class="header-app d-flex flex-column flex-md-row justify-content-between gap-3 align-items-md-center">
                <div>
                    <h4 class="fw-bold mb-0">Eventos Auditoria</h4>
                </div>
                ...
                <div class="d-flex gap-2 align-items-center">
                    <input
                        ...
                        wire:model="search"
                    >
                    <button class="btn btn-outline-light2" type="button" wire:click="searchEventosAuditoria">Buscar</button>
                        ...
                        <tbody>
                            @forelse ($registros as $registro)
                                <tr>
                                    ...
                                    <td>{{ optional($registro->auditoria)->nombre_evento ?? '-' }}</td>
    ...
    <div class="modal fade" id="eventosAuditoriaModal" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <form wire:submit.prevent="save">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{ $editingId ? 'Editar evento auditoria' : 'Nuevo evento auditoria' }}
                        </h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <select wire:model.defer="auditoria_id" class="form-control">
                                <option value="">Seleccione...</option>
                                @foreach ($auditorias as $auditoria)
                                    <option value="{{ $auditoria->id }}">{{ $auditoria->nombre_evento }}</option>
                                @endforeach
                            </select>
                            @error('auditoria_id') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
...
<script>
    window.addEventListener('openEventosAuditoriaModal', () => {
        $('#eventosAuditoriaModal').modal('show');
    });

    window.addEventListener('closeEventosAuditoriaModal', () => {
