<?php

return [
    /*
    |--------------------------------------------------------------------------
    | Route Permission Middleware
    |--------------------------------------------------------------------------
    |
    | If enabled, every authenticated route with a name can be protected by
    | a permission with the same route name (for example: users.index).
    |
    */
    'route_permission' => [
        'enabled' => true,
        'allow_when_permission_missing' => false,
    ],

    'forbidden_redirect_url' => env(
        'FORBIDDEN_REDIRECT_URL',
        'https://trackingbo.correos.gob.bo:8100/inicio'
    ),

    /*
    |--------------------------------------------------------------------------
    | Super Admin Role
    |--------------------------------------------------------------------------
    |
    | This role bypasses route permission checks.
    |
    */
    'super_admin_role' => 'administrador',

    /*
    |--------------------------------------------------------------------------
    | Excluded Route Names
    |--------------------------------------------------------------------------
    |
    | These route names are excluded from automatic permission discovery and
    | from route-level permission checks.
    |
    */
    'excluded_route_permissions' => [
        'welcome',
        'tracking.demo',
        'api.busqueda.ems-eventos',
        'api.busqueda.captcha',
        'api.public.zona-paquete',
        'login',
        'register',
        'password.request',
        'password.email',
        'password.reset',
        'password.store',
        'password.confirm',
        'password.update',
        'verification.notice',
        'verification.verify',
        'verification.send',
        'clientes.login',
        'auth.google.redirect',
        'auth.google.callback',
        'logout',
        'role-has-permissions.index',
        'role-has-permissions.create',
        'role-has-permissions.store',
        'role-has-permissions.edit',
        'role-has-permissions.update',
        'role-has-permissions.destroy',
        'profile.edit',
        'profile.update',
        'profile.destroy',
        'paquetes-ips.index',
        'ips.index',
        'ips.selection.add',
        'ips.selection.remove',
        'ips.operate',
        'paquetes-ips.deliver',
        'recepcion-ips.index',
        'recepcion-ips.receive',
        'users.impersonate',
        'users.impersonate.destroy',
        'acl.livewire-actions',
        'paquetes-certificados.baja-pdf',
        'paquetes-certificados.rezago-pdf',
        'api.carteros.distribucion',
        'api.carteros.asignados',
        'api.carteros.cartero',
        'api.carteros.provincia',
        'api.carteros.devolucion',
        'api.carteros.domicilio',
        'api.carteros.users',
        'api.carteros.asignar',
        'api.carteros.cambiar-cartero',
        'api.carteros.registrar-guia',
        'api.carteros.devolver-almacen',
        'api.carteros.aceptar-paquetes',
        'api.carteros.dar-baja',
        'carteros.distribucion.report',
        'carteros.asignados.report',
        'tarifario.pdf',
        'tarifario.global-excel',
        'despachos.expedicion.pdf',
        'ventas-sucursal.export.pdf',
    ],

    /*
    |--------------------------------------------------------------------------
    | Custom Permissions
    |--------------------------------------------------------------------------
    |
    | Add non-route permissions here (for button-only actions, etc).
    |
    */
    'custom_permissions' => [
        'pulse',
        'feature.carteros.distribucion.assign',
        'feature.carteros.distribucion.selfassign',
        'feature.carteros.asignados.change',
        'feature.carteros.asignados.report',
        'feature.carteros.cartero.guide',
        'feature.carteros.cartero.province',
        'feature.carteros.cartero.deliver',
        'feature.carteros.cartero.returnwindow',
        'feature.carteros.devolucion.restore',
        'feature.carteros.entrega.deliver',
        'feature.carteros.entrega.attempt',
        'feature.paquetes-contrato.index.report',
        'feature.paquetes-contrato.index.print',
        'feature.paquetes-contrato.index.create',
        'feature.paquetes-contrato.index.manage',
        'feature.paquetes-contrato.gestor.search',
        'feature.paquetes-contrato.gestor.view',
        'feature.paquetes-contrato.gestor.export',
        'feature.paquetes-contrato.gestor.print',
        'feature.paquetes-contrato.gestor.report',
        'feature.paquetes-contrato.almacen.report',
        'feature.paquetes-contrato.almacen.print',
        'feature.paquetes-contrato.recoger-envios.print',
        'feature.paquetes-contrato.cartero.print',
        'feature.paquetes-contrato.create.create',
        'feature.paquetes-contrato.create.manage',
        'feature.paquetes-contrato.create-con-tarifa.create',
        'feature.paquetes-contrato.entregados.print',
        'feature.paquetes-contrato.entregados.export',
        'feature.paquetes-ems.contrato-rapido.create.create',
        'feature.paquetes-ems.contrato-rapido.create.save',
        'feature.paquetes-ems.contrato-rapido.create.delete',
        'feature.paquetes-ems.almacen.registercontract',
        'feature.paquetes-ems.almacen.weighcontract',
        'feature.paquetes-ems.almacen.weightiktoker',
        'feature.paquetes-ems.almacen.sendventanilla',
        'feature.paquetes-ems.almacen.sendregional',
        'feature.paquetes-ems.almacen.reprintcn33',
        'feature.paquetes-ems.en-transito.reprintcn33',
        'feature.paquetes-ems.encargado.cancel',
        'feature.paquetes-ems.encargado.returnorigin',
        'feature.paquetes-ems.encargado.returndestination',
        'feature.paquetes-ems.encargado.returnwindow',
        'feature.paquetes-ems.encargado.updateweight',
        'feature.paquetes-ems.encargado.changecartero',
        'feature.paquetes-ems.encargado.print',
        'feature.paquetes-ems.entregados.deliver',
        'feature.paquetes-ems.devolucion.deliver',
        'feature.paquetes-ems.devolucion.print',
        'feature.users.empresas.create',
        'feature.users.empresas.edit',
        'feature.users.empresas.delete',
        'feature.users.empresas.restore',
        'feature.users.empresas.export',
        'feature.users.empresas.manage',
        'feature.empresas.export',
        'feature.empresas.history',
        'feature.empresas.historial.search',
        'feature.empresas.historial.view-pdf',
        'feature.alertas-empresa.view',
        'feature.alertas-empresa.create',
        'feature.alertas-empresa.approve',
        'feature.alertas-empresa.readers',
        'feature.alertas-empresa.delete',
        'feature.alertas-empresa.export',
        'feature.alertas-empresa.manage',
        'alertas-empresa.index',
        'alertas-empresa.store',
        'alertas-empresa.approve',
        'alertas-empresa.destroy',
        'alertas-empresa.portada',
        'alertas-empresa.pdf',
        'alertas-empresa.read',
        'log-viewer.index',
        'feature.paquetes-ems.solicitudes.index.create',
        'feature.paquetes-ems.solicitudes.index.assign',
        'feature.paquetes-ems.solicitudes.index.print',
        'feature.despachos.abiertos.create',
        'feature.despachos.abiertos.edit',
        'feature.despachos.abiertos.delete',
        'feature.despachos.abiertos.opensacas',
        'feature.despachos.abiertos.confirm',
        'feature.despachos.abiertos.restore',
        'feature.despachos.expedicion.print',
        'feature.despachos.expedicion.confirm',
        'feature.despachos.expedicion.restore',
        'feature.despachos.expedicion.edit',
        'feature.despachos.admitidos.assign',
        'feature.despachos.admitidos.confirm',
        'feature.dashboard.facturacion',
        'feature.sacas.index.create',
        'feature.sacas.index.edit',
        'feature.sacas.index.delete',
        'feature.sacas.index.assign',
        'feature.sacas.index.confirm',
        'feature.tarifa-contrato.create',
        'feature.tarifa-contrato.duplicate',
        'feature.tarifa-contrato.edit',
        'feature.tarifa-contrato.delete',
        'feature.tarifa-contrato.save',
        'feature.tarifa-contrato.import',
        'feature.tarifa-contrato.export',
        'feature.tarifa-contrato.report',
        'feature.bitacoras.index.create',
        'feature.bitacoras.index.save',
        'feature.bitacoras.index.edit',
        'feature.bitacoras.index.delete',
        'feature.bitacoras.index.export',
        'feature.bitacoras.create.create',
        'feature.bitacoras.create.save',
        'feature.todos-paquetes.index.create',
        'feature.empresa.guias.index.search',
        'feature.empresa.guias.index.export',
        'feature.empresa.guias.index.view',
        'feature.empresa.guias.index.print',
        'dashboard.dir-operaciones',
        'dashboard.dir-comercial',
        'dashboard.conciliacion',
        'dashboard.conciliacion.facturado',
        'dashboard.conciliacion.empresas-paquetes',
        'dashboard.conciliacion.conciliaciones',
        'feature.conciliacion.conciliaciones.conciliar',
        'feature.conciliacion.conciliaciones.por-cobrar',
        'feature.conciliacion.conciliaciones.pago-recibido',
        'feature.conciliacion.conciliaciones.confirmacion-pago',
        'feature.conciliacion.conciliaciones.editar',
        'feature.conciliacion.conciliaciones.conciliado',
        'dashboard.dir-financiera',
        'dashboard.financiera.ventas-servicios',
        'dashboard.financiera.flujo-cajero',
        'dashboard.financiera.ventas-servicios.detalle',
        'dashboard.financiera.panel',
        'dashboard.financiera.presupuesto',
        'dashboard.financiera.ingresos-egresos',
        'dashboard.financiera.cuentas-cobrar',
        'dashboard.financiera.indicadores',
    ],

    /*
    |--------------------------------------------------------------------------
    | Automatic ACL Sync
    |--------------------------------------------------------------------------
    |
    | Keeps route and action permissions synchronized in production without
    | requiring manual seeding after each deployment.
    |
    */
    'sync' => [
        'enabled' => true,
        'ttl_seconds' => 300,
    ],

    /*
    |--------------------------------------------------------------------------
    | UI Behaviour
    |--------------------------------------------------------------------------
    */
    'ui' => [
        'auto_hide_livewire_actions' => true,
    ],

    /*
    |--------------------------------------------------------------------------
    | Default Roles
    |--------------------------------------------------------------------------
    */
    'default_roles' => [
        'administrador',
        'regional',
        'taller',
        'conductor',
        'empresa',
    ],

    /*
    |--------------------------------------------------------------------------
    | Legacy Role Aliases
    |--------------------------------------------------------------------------
    |
    | Maps legacy role names from the previous ACL to the canonical role names
    | used by the current permission templates.
    |
    */
    'legacy_role_aliases' => [
        'ADMINISTRADOR' => 'administrador',
        'ENCARGADO EMS' => 'encargado',
        'ENCARGADO URBANO' => 'encargado',
        'ENCARGADO TRATAMIENTO' => 'encargado',
        'AUXILIAR URBANO' => 'auxiliar',
        'AUXILIAR URBANO 7' => 'auxiliar',
        'AUXILIAR TRATAMIENTO' => 'auxiliar',
    ],

    /*
    |--------------------------------------------------------------------------
    | Initial Role Templates (Wildcard Patterns)
    |--------------------------------------------------------------------------
    |
    | Used by AclSeeder to grant initial permissions. Roles with existing
    | permissions are not overwritten (except super admin).
    |
    */
    'role_templates' => [
        // En web mantenemos solo el rol tecnico base.
        // Los roles operativos se administran manualmente desde el panel.
        'administrador' => ['*'],
        'regional' => [
            'livewire.vehicle-brands',
            'feature.vehicle-brands.*',
            'livewire.vehicles',
            'feature.vehicles.*',
            'livewire.drivers',
            'feature.drivers.*',
            'drivers.memorandum.download',
            'livewire.vehicle-assignments',
            'feature.vehicle-assignments.*',
            'vehicle-assignments.report.pdf',
            'livewire.vehicle-logs',
            'feature.vehicle-logs.*',
            'vehicle-logs.*',
            'map.index',
            'livewire.map',
            'feature.map.*',
            'map.data',
            'livewire.maintenance-types',
            'feature.maintenance-types.*',
            'livewire.maintenance-appointments',
            'feature.maintenance-appointments.*',
            'maintenance-appointments.*',
            'livewire.maintenance-calendar',
            'feature.maintenance-calendar.*',
            'livewire.maintenance-alerts',
            'feature.maintenance-alerts.*',
            'livewire.maintenance-logs',
            'feature.maintenance-logs.*',
            'maintenance-logs.*',
            'livewire.workshops',
            'feature.workshops.*',
            'workshops.*',
            'vehicles.maintenance-report',
            'livewire.fuel-logs',
            'feature.fuel-logs.*',
            'fuel-logs.*',
        ],
        'taller' => [
            'livewire.workshops',
            'feature.workshops.*',
            'workshops.*',
        ],
        'conductor' => [
            'livewire.vehicles',
            'feature.vehicles.view',
            'livewire.drivers',
            'livewire.vehicle-logs',
            'feature.vehicle-logs.*',
            'vehicle-logs.*',
            'map.index',
            'livewire.map',
            'feature.map.*',
            'map.data',
            'livewire.fuel-logs',
            'feature.fuel-logs.*',
            'fuel-logs.*',
            'livewire.maintenance-alerts',
            'feature.maintenance-alerts.*',
            'livewire.maintenance-calendar',
            'feature.maintenance-calendar.*',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Optional Module Labels
    |--------------------------------------------------------------------------
    */
    'module_labels' => [
        'users' => 'Usuarios',
        'users.empresas' => 'Usuarios empresas',
        'roles' => 'Roles',
        'permissions' => 'Permisos',
        'role-has-permissions' => 'Accesos',
        'dashboard' => 'Dashboard',
        'dashboard.conciliacion.conciliaciones' => 'Conciliaciones',
        'conciliacion.conciliaciones' => 'Conciliaciones',
        'dashboard.global-ingreso' => 'Dir. Operaciones - Global Ingreso',
        'dashboard.global-por-servicio' => 'Dir. Operaciones - Global por servicio',
        'dashboard.reimprimir-cn33' => 'Dir. Operaciones - Reimprimir CN-33',
        'dashboard.generacion-cn' => 'Dir. Operaciones - Generacion de CN',
        'dashboard.marbetes' => 'Dir. Operaciones - Marbetes CN 35',
        'dashboard.comercial.rendimiento-servicios' => 'Dir. Comercial - Rendimiento Servicios',
        'reportes' => 'Reportes',
        'malencaminados' => 'Malencaminados',
        'bastiones' => 'Bastiones',
        'mis-ventas' => 'Mis Ventas',
        'ventas-sucursal' => 'Ventas Sucursal',
        'paquetes-ems' => 'Paquetes EMS',
        'paquetes-ems.almacen-admisiones' => 'Almacen admisiones',
        'paquetes-ems.solicitudes.index' => 'Solicitudes EMS',
        'paquetes-certificados' => 'Paquetes Certificados',
        'paquetes-ordinarios' => 'Paquetes Ordinarios',
        'paquetes-contrato' => 'Paquetes Contrato',
        'paquetes-contrato.gestor' => 'Gestor contratos',
        'area-contratos' => 'Area Contratos',
        'area-contratos.contratos-escaneados' => 'Contratos escaneados',
        'empresa.guias' => 'Guías Empresa',
        'empresa.guias.index' => 'Guías Empresa',
        'indicadores' => 'Indicadores',
        'despachos' => 'Despachos',
        'sacas' => 'Sacas',
        'carteros' => 'Carteros',
        'api.carteros' => 'API Carteros',
        'empresas' => 'Empresas',
        'empresas.historial' => 'Historial de empresas',
        'empresas.historial.index' => 'Historial de empresas',
        'alertas-empresa' => 'Alertas para empresas',
        'codigo-empresa' => 'Codigo Empresa',
        'eventos' => 'Eventos',
        'eventos-ems' => 'Eventos EMS',
        'eventos-certi' => 'Eventos Certificados',
        'eventos-ordi' => 'Eventos Ordinarios',
        'eventos-despacho' => 'Eventos Despachos',
        'eventos-contrato' => 'Eventos Contratos',
        'eventos-ips' => 'Eventos IPS',
        'users.ips-links' => 'Vinculacion IPS',
        'eventos-auditoria' => 'Eventos Auditoria',
        'auditoria' => 'Auditoria',
        'servicios' => 'Servicios',
        'conceptos-facturacion' => 'Conceptos Facturables',
        'origenes' => 'Origenes',
        'destinos' => 'Destinos',
        'pesos' => 'Pesos',
        'tarifario' => 'Tarifario',
        'tarifa-contrato' => 'Tarifa Contrato',
        'bitacoras' => 'Bitacoras',
        'importar' => 'Importaciones',
        'backups' => 'Respaldos',
    ],
];
